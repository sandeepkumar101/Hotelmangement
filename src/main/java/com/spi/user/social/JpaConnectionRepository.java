...
import com.spi.domain.SocialUserBuilder;
...
public class JpaConnectionRepository implements ConnectionRepository {
	...
	private final SocialUserDAO socialUserRepository;
	private final UserDAO userRepository;
	private final User user;
	private final ConnectionFactoryLocator connectionFactoryLocator;
	private final TextEncryptor textEncryptor;

	public JpaConnectionRepository(SocialUserDAO socialUserRepository,
			UserDAO userRepository, User user,
			ConnectionFactoryLocator locator, TextEncryptor encryptor) {
	...
	public MultiValueMap<String, Connection<?>> findAllConnections() {
		List<Connection<?>> resultList = connectionMapper
				.mapEntities(socialUserRepository.findAllByUser(user));
	...
	public List<Connection<?>> findConnections(String providerId) {
		return connectionMapper.mapEntities(socialUserRepository
				.findByUserAndProviderId(user, providerId));
	}
	...
	public MultiValueMap<String, Connection<?>> findConnectionsToUsers(
		...
		List<Connection<?>> resultList = connectionMapper
				.mapEntities(socialUserRepository.findByUserAndProviderUserId(
						user, providerUsers));
	...
	public Connection<?> getConnection(ConnectionKey connectionKey) {
		try {
			return connectionMapper.mapEntity(socialUserRepository
					.findByUserAndProviderIdAndProviderUserId(user,
	...
	public void addConnection(Connection<?> connection) {
		try {
			...
			// TODO: currently only support 1 connection per user per provider
			// (rank = 1)
			int rank = 1;
			// create a SocialUser and call save
			SocialUser socialUser = SocialUserBuilder.create().withUser(user)
	...
	public void removeConnections(String providerId) {
		List<SocialUser> users = socialUserRepository.findByUserAndProviderId(
				user, providerId);
	...
	private Connection<?> findPrimaryConnection(String providerId) {
		List<Connection<?>> connections = connectionMapper
				.mapEntities(socialUserRepository.findByUserAndProviderId(user,
						providerId));
