...
import com.spi.user.exception.AuthenticationException;
import com.spi.user.exception.DuplicateUserException;
import com.spi.exception.ValidationException;
import com.spi.user.api.*;
...
import static org.hamcMatchers.*;
...
//        when(routeService.getRouteDetails(any(ExternalUser.class), any(String.class))).thenReturn(new ExternalUser(TEST_USER, AUTH_TOKEN));
//        ClientResponse response = super.resource().path("user/" + TEST_USER.getUuid().toString()).accept(APPLICATION_JSON).get(ClientResponse.class);
//        assertThat(response.getStatus(), is(200));
//        ExternalUser userResponse = response.getEntity(ExternalUser.class);
//        assertThat(userResponse.getEmailAddress(), is(TEST_USER.getEmailAddress()));
